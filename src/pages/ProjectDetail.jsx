        ...
        <div className={`min-h-screen animate-fade-in relative flex flex-col ${theme.bg} pb-24 md:pb-0`}>
            ...
            <div className="flex-1 w-full max-w-[1920px] mx-auto py-12 flex flex-col gap-20">
                ...
                {typeof displayContent.process === 'object' && displayContent.process.type === 'rich' ? (
                    <div className="w-full">
                        ...
                        {displayContent.process.sections.map((section, idx) => {
                            if (section.type === 'text') {
                                ...
                                const showImage = !displayContent.process.renderComponent && (section.video || section.iframe || section.image || (project.images && project.images[2] && project.id !== 3 && !section.hideImage));
                                ...
                                return (
                                    <div key={idx} className={`max-w-6xl mx-auto px-6 ${showImage ? 'grid md:grid-cols-2 gap-12 items-center' : 'text-center'} mb-12`}>
                                        ...
                                        {showImage && (
                                            <div className={`${section.video ? 'w-full aspect-video' : 'aspect-square'} ${section.video && project.id === 1 ? 'overflow-visible pt-16' : 'overflow-hidden'} ${(project.id === 2 || project.id === 3 || project.id === 1) ? '' : 'shadow-sm'} ${(project.id === 3 || project.id === 1) ? (isWandering ? 'bg-charcoal' : 'bg-cream') : theme.imagePlaceholderBg} ${section.video ? '' : 'cursor-zoom-in'}`} onClick={section.video ? undefined : () => setSelectedImage(section.image || project.images[2])}>
                                                {section.video ? (
                                                    <video
                    ...
                    <div className={`w-full max-w-6xl mx-auto px-6 grid ${project.images && project.images[2] || (project.id === 4 && !isWandering && displayContent.keyTakeaway?.processImages) ? 'md:grid-cols-2' : 'grid-cols-1'} gap-12 items-center`}>
